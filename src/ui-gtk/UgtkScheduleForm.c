...
static gboolean on_button_press_event(GtkWidget* widget, GdkEventMotion* event,
                                      struct UgtkScheduleForm* sform);
static gboolean on_motion_notify_event(GtkWidget* widget, GdkEventMotion* event,
                                       struct UgtkScheduleForm* sform);
static gboolean on_leave_notify_event(GtkWidget* menu, GdkEventCrossing* event,
                                      struct UgtkScheduleForm* sform);


void
ugtk_schedule_form_init(struct UgtkScheduleForm* sform)
{
    ...
    g_signal_connect(widget, "toggled", G_CALLBACK(on_enable_toggled), sform);
    sform->enable = widget;
    ...
    gtk_widget_set_size_request(widget, UgtkGrid.width_all + text_width + 32, UgtkGrid.height_all);
    gtk_widget_add_events(widget,
                          GDK_BUTTON_PRESS_MASK | GDK_POINTER_MOTION_MASK | GDK_LEAVE_NOTIFY_MASK);
    g_signal_connect(widget, "draw", G_CALLBACK(on_draw_callback), sform);
    g_signal_connect(widget, "button-press-event", G_CALLBACK(on_button_press_event), sform);
    g_signal_connect(widget, "motion-notify-event", G_CALLBACK(on_motion_notify_event), sform);
    g_signal_connect(widget, "leave-notify-event", G_CALLBACK(on_leave_notify_event), sform);
...
void
ugtk_schedule_form_get(struct UgtkScheduleForm* sform, UgtkSetting* setting)
{
    //	gint  value;

    memcpy(setting->scheduler.state.at, sform->state, sizeof(sform->state));
    setting->scheduler.enable = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(sform->enable));
...
void
ugtk_schedule_form_set(struct UgtkScheduleForm* sform, UgtkSetting* setting)
{
    //	gint  value;
...
//
static void
on_enable_toggled(GtkToggleButton* togglebutton, struct UgtkScheduleForm* sform)
{
    gboolean active;
...
static gboolean
on_draw_callback(GtkWidget* widget, cairo_t* cr, struct UgtkScheduleForm* sform)
{
    gboolean sensitive;
    gint y, x;
    gdouble cy, cx, ox;
    ...
    // fill grid
    if (sensitive == FALSE) {
        cairo_set_source_rgb(cr, COLOR_DISABLE_R, COLOR_DISABLE_G, COLOR_DISABLE_B);
    }
    for (cy = 1.5, y = 0; y < 7; y++, cy += UgtkGrid.height_and_line) {
        for (cx = 1.5 + ox, x = 0; x < 24; x++, cx += UgtkGrid.width_and_line) {
            if (sensitive) {
                cairo_set_source_rgb(cr, colors[sform->state[y][x]][0],
                                     colors[sform->state[y][x]][1], colors[sform->state[y][x]][2]);
            }
            cairo_rectangle(cr, cx, cy, UgtkGrid.width - 0.5, UgtkGrid.height - 0.5);
            cairo_fill(cr);
        }
    }
...
static gboolean
on_button_press_event(GtkWidget* widget, GdkEventMotion* event, struct UgtkScheduleForm* sform)
{
    gint x, y;
    ...
    state = (sform->state[y][x] == UGTK_SCHEDULE_TURN_OFF) ? UGTK_SCHEDULE_NORMAL
    ...
    // Queue a redraw instead of using deprecated gdk_cairo_create
...
static gboolean
on_motion_notify_event(GtkWidget* widget, GdkEventMotion* event, struct UgtkScheduleForm* sform)
{
    gint x, y;
    gchar* string;
    GdkWindow* gdkwin;
    ...
    // update time_tips
    string = g_strdup_printf("%s, %.2d:00 - %.2d:59", gettext(week_days[y]), x, x);
...
static gboolean
on_leave_notify_event(GtkWidget* menu, GdkEventCrossing* event, struct UgtkScheduleForm* sform)
{
    gtk_label_set_text(sform->time_tips, "");
    return TRUE;
}
...
//
static void
ugtk_grid_global_init(int width, int height)
{
...
static GtkWidget*
ugtk_grid_new(const gdouble* rgb_array)
{
    GtkWidget* widget;
    ...
    g_signal_connect(widget, "draw", G_CALLBACK(ugtk_grid_draw), (gpointer) rgb_array);

    return widget;
}

static gboolean
ugtk_grid_draw(GtkWidget* widget, cairo_t* cr, const gdouble* rgb_array)
{
    ...
    if (gtk_widget_get_sensitive(widget)) {
        cairo_set_source_rgb(cr, rgb_array[0], rgb_array[1], rgb_array[2]);
    } else {
        cairo_set_source_rgb(cr, COLOR_DISABLE_R, COLOR_DISABLE_G, COLOR_DISABLE_B);
    }
