...
ugtk_app_init_ui(UgtkApp* app)
{
    ...
    gtk_accel_map_add_entry(UGTK_APP_ACCEL_PATH_SAVE_ALL, GDK_KEY_s,
...
// set default size
static void
ugtk_app_init_size(UgtkApp* app)
{
    GdkDisplay* display;
    GdkMonitor* monitor;
...
#if defined _WIN32 || defined _WIN64
static void
ugtk_app_init_ui_win32(UgtkApp* app, int screen_width)
{
    GSettings* gset;
    gint sidebar_width;
...
static void
ugtk_window_init_ui(struct UgtkWindow* window, UgtkApp* app)
{
    GtkBox* vbox;
...
//
static void
ugtk_statusbar_init_ui(struct UgtkStatusbar* sbar)
{
    GtkBox* hbox;
    GtkWidget* widget;
...
//
static void
ugtk_toolbar_init_ui(struct UgtkToolbar* ugt, GtkAccelGroup* accel_group)
{
    GtkToolbar* toolbar;
    ...
    // New button --- start ---
    image = gtk_image_new_from_icon_name("document-new-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    ...
    menu_item = gtk_menu_item_new_with_mnemonic(_("New _URL Sequence batch..."));
    gtk_menu_shell_append((GtkMenuShell*) menu, menu_item);
    ...
    image = gtk_image_new_from_icon_name("document-save-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    ...
    image =
        gtk_image_new_from_icon_name("media-playback-pause-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    tool_item = (GtkToolItem*) gtk_tool_button_new(image, NULL);
    ...
    image = gtk_image_new_from_icon_name("go-up-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    tool_item = (GtkToolItem*) gtk_tool_button_new(image, NULL);
    gtk_tool_item_set_tooltip_text(tool_item, _("Move selected download up"));
    gtk_toolbar_insert(toolbar, tool_item, -1);
    ugt->move_up = GTK_WIDGET(tool_item);

    image = gtk_image_new_from_icon_name("go-down-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    tool_item = (GtkToolItem*) gtk_tool_button_new(image, NULL);
    gtk_tool_item_set_tooltip_text(tool_item, _("Move selected download down"));
    gtk_toolbar_insert(toolbar, tool_item, -1);
    ugt->move_down = GTK_WIDGET(tool_item);

    image = gtk_image_new_from_icon_name("go-top-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    tool_item = (GtkToolItem*) gtk_tool_button_new(image, NULL);
    gtk_tool_item_set_tooltip_text(tool_item, _("Move selected download to top"));
    gtk_toolbar_insert(toolbar, tool_item, -1);
    ugt->move_top = GTK_WIDGET(tool_item);

    image = gtk_image_new_from_icon_name("go-bottom-symbolic", GTK_ICON_SIZE_SMALL_TOOLBAR);
    tool_item = (GtkToolItem*) gtk_tool_button_new(image, NULL);
