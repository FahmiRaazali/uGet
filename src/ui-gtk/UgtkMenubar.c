...
// UgtkFileMenu
static void
on_create_download(GtkWidget* widget, UgtkApp* app)
{
    ugtk_app_create_download(app, NULL, NULL);
}

static void
on_offline_mode(GtkWidget* widget, UgtkApp* app)
{
    UgetNode* cnode;
    gboolean offline;
    ...
    // trayicon menu
    offline = gtk_check_menu_item_get_active((GtkCheckMenuItem*) app->trayicon.menu.offline_mode);
    if (offline != app->setting.offline_mode) {
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->trayicon.menu.offline_mode,
...
// UgtkEditMenu
static void
on_clipboard_monitor(GtkWidget* widget, UgtkApp* app)
{
    gboolean active;
    ...
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->trayicon.menu.clipboard_monitor,
                                   active);
    app->trayicon.menu.emission = TRUE;
}

static void
on_clipboard_quiet(GtkWidget* widget, UgtkApp* app)
{
    gboolean active;
    ...
    app->trayicon.menu.emission = FALSE;
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->trayicon.menu.clipboard_quiet, active);
    app->trayicon.menu.emission = TRUE;
}

static void
on_commandline_quiet(GtkWidget* widget, UgtkApp* app)
{
    gboolean active;
    ...
    app->trayicon.menu.emission = FALSE;
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->trayicon.menu.commandline_quiet,
                                   active);
    app->trayicon.menu.emission = TRUE;
}

static void
on_skip_existing(GtkWidget* widget, UgtkApp* app)
{
    gboolean active;
    ...
    app->trayicon.menu.emission = FALSE;
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->trayicon.menu.skip_existing, active);
    app->trayicon.menu.emission = TRUE;
}

static void
on_apply_recent(GtkWidget* widget, UgtkApp* app)
{
    gboolean active;
    ...
    app->trayicon.menu.emission = FALSE;
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->trayicon.menu.apply_recent, active);
    app->trayicon.menu.emission = TRUE;
}

static void
on_config_completion(GtkWidget* widget, UgtkApp* app)
{
...
static void
on_config_completion_remember(GtkWidget* widget, UgtkApp* app)
{
    gboolean remember;
...
static void
on_config_completion_help(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/documentation/on-complete");
}

static void
on_config_settings(GtkWidget* widget, UgtkApp* app)
{
...
//
static void
on_change_visible_widget(GtkWidget* widget, UgtkApp* app)
{
...
static void
on_change_visible_summary(GtkWidget* widget, UgtkApp* app)
{
...
static void
on_change_visible_column(GtkWidget* widget, UgtkApp* app)
{
...
static void
on_move_category_up(GtkWidget* widget, UgtkApp* app)
{
    UgetNode* cnode;
    ...
    ugtk_traveler_select_category(&app->traveler, app->traveler.category.cursor.pos - 1, -1);
    gtk_widget_queue_draw((GtkWidget*) app->traveler.category.view);
    // emit signal "row-changed"
    path1 = gtk_tree_model_get_path(GTK_TREE_MODEL(app->traveler.category.model), &iter1);
    path2 = gtk_tree_model_get_path(GTK_TREE_MODEL(app->traveler.category.model), &iter2);
    gtk_tree_model_row_changed(GTK_TREE_MODEL(app->traveler.category.model), path1, &iter1);
    gtk_tree_model_row_changed(GTK_TREE_MODEL(app->traveler.category.model), path2, &iter2);
    gtk_tree_path_free(path1);
    gtk_tree_path_free(path2);
}

static void
on_move_category_down(GtkWidget* widget, UgtkApp* app)
{
    UgetNode* cnode;
    GtkTreeIter iter1, iter2;
    GtkTreePath *path1, *path2;
...
static void
on_delete_category(GtkWidget* widget, UgtkApp* app)
{
...
static void
on_delete_download(GtkWidget* widget, UgtkApp* app)
{
    ugtk_app_delete_download(app, FALSE);
}

static void
on_delete_download_file(GtkWidget* widget, UgtkApp* app)
{
    UgtkConfirmDialog* cdialog;
...
static void
on_open_download_file(GtkWidget* widget, UgtkApp* app)
{
    UgetCommon* common;
    UgetNode* node;
    GtkWidget* dialog;
    ...
    if (ugtk_launch_default_app(common->folder, common->file) == FALSE) {
        string =
            g_strdup_printf(_("Can't launch default application for file '%s'."), common->file);
        dialog = gtk_message_dialog_new(app->window.self, GTK_DIALOG_DESTROY_WITH_PARENT,
...
static void
on_open_download_folder(GtkWidget* widget, UgtkApp* app)
{
    UgetCommon* common;
    UgetNode* node;
    GtkWidget* dialog;
...
#if defined _WIN32 || defined _WIN64
    {
        ...
        if (common->file == NULL && common->uri) {
            ...
            if (uuri.file == -1)
                ...
                argument = g_strndup(uuri.uri + uuri.file, ug_uri_file(&uuri, NULL));
            }
...
static void
on_set_download_force_start(GtkWidget* widget, UgtkApp* app)
{
    UgetNode* node;
    UgetNode* cursor;
    GList* list;
...
static void
on_set_download_runnable(GtkWidget* widget, UgtkApp* app)
{
...
static void
on_move_download(GtkWidget* widget, UgtkApp* app)
{
    GPtrArray* array;
    UgetNode* cnode;
    UgetNode* dnode;
...
static void
on_set_download_prioriy(GtkWidget* widget, UgtkApp* app)
{
...
//
static void
on_help_online(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/help");
}

static void
on_documentation(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/documentation");
}

static void
on_support_forum(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/forum/");
}

static void
on_submit_feedback(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/feedback");
}

static void
on_report_bug(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/reportbug");
}

static void
on_keyboard_shortcuts(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/keyboard-shortcuts");
}

static void
on_check_updates(GtkWidget* widget, UgtkApp* app)
{
    ugtk_launch_uri("http://ugetdm.com/versioncheck?v=" PACKAGE_VERSION);
}

static void
on_about(GtkWidget* widget, UgtkApp* app)
{
...
void
ugtk_menubar_init_callback(UgtkMenubar* menubar, UgtkApp* app)
{
    // ----------------------------------------------------
    // UgtkFileMenu
    g_signal_connect(menubar->file.create_download, "activate", G_CALLBACK(on_create_download),
                     app);
    ...
    g_signal_connect_swapped(menubar->file.save, "activate", G_CALLBACK(ugtk_app_save), app);
    g_signal_connect(menubar->file.offline_mode, "toggled", G_CALLBACK(on_offline_mode), app);
    g_signal_connect_swapped(menubar->file.quit, "activate", G_CALLBACK(ugtk_app_decide_to_quit),
                             app);

    // ----------------------------------------------------
    // UgtkEditMenu
    g_signal_connect(menubar->edit.clipboard_monitor, "activate", G_CALLBACK(on_clipboard_monitor),
                     app);
    g_signal_connect(menubar->edit.clipboard_quiet, "activate", G_CALLBACK(on_clipboard_quiet),
                     app);
    g_signal_connect(menubar->edit.commandline_quiet, "activate", G_CALLBACK(on_commandline_quiet),
                     app);
    g_signal_connect(menubar->edit.skip_existing, "activate", G_CALLBACK(on_skip_existing), app);
    g_signal_connect(menubar->edit.apply_recent, "activate", G_CALLBACK(on_apply_recent), app);

    g_signal_connect(menubar->edit.completion.disable, "activate", G_CALLBACK(on_config_completion),
                     app);
    g_signal_connect(menubar->edit.completion.hibernate, "activate",
                     G_CALLBACK(on_config_completion), app);
    g_signal_connect(menubar->edit.completion.suspend, "activate", G_CALLBACK(on_config_completion),
                     app);
    g_signal_connect(menubar->edit.completion.shutdown, "activate",
                     G_CALLBACK(on_config_completion), app);
    g_signal_connect(menubar->edit.completion.reboot, "activate", G_CALLBACK(on_config_completion),
                     app);
    g_signal_connect(menubar->edit.completion.custom, "activate", G_CALLBACK(on_config_completion),
                     app);
    ...
    g_signal_connect(menubar->edit.settings, "activate", G_CALLBACK(on_config_settings), app);

    // ----------------------------------------------------
    // UgtkViewMenu
    g_signal_connect(menubar->view.toolbar, "toggled", G_CALLBACK(on_change_visible_widget), app);
    g_signal_connect(menubar->view.statusbar, "toggled", G_CALLBACK(on_change_visible_widget), app);
    g_signal_connect(menubar->view.category, "toggled", G_CALLBACK(on_change_visible_widget), app);
    g_signal_connect(menubar->view.summary, "toggled", G_CALLBACK(on_change_visible_widget), app);
    // summary items
    ...
    g_signal_connect(menubar->view.columns.total, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.percent, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.elapsed, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.left, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.speed, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.upload_speed, "toggled",
                     G_CALLBACK(on_change_visible_column), app);
    g_signal_connect(menubar->view.columns.uploaded, "toggled",
                     G_CALLBACK(on_change_visible_column), app);
    g_signal_connect(menubar->view.columns.ratio, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.retry, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    g_signal_connect(menubar->view.columns.category, "toggled",
                     G_CALLBACK(on_change_visible_column), app);
    g_signal_connect(menubar->view.columns.uri, "toggled", G_CALLBACK(on_change_visible_column),
                     app);
    ...
    g_signal_connect_swapped(menubar->category.create, "activate",
                             G_CALLBACK(ugtk_app_create_category), app);
    g_signal_connect(menubar->category.delete, "activate", G_CALLBACK(on_delete_category), app);
    g_signal_connect_swapped(menubar->category.properties, "activate",
                             G_CALLBACK(ugtk_app_edit_category), app);
    g_signal_connect(menubar->category.move_up, "activate", G_CALLBACK(on_move_category_up), app);
    g_signal_connect(menubar->category.move_down, "activate", G_CALLBACK(on_move_category_down),
                     app);

    // ----------------------------------------------------
    // UgtkDownloadMenu
    g_signal_connect(menubar->download.create, "activate", G_CALLBACK(on_create_download), app);
    g_signal_connect(menubar->download.delete, "activate", G_CALLBACK(on_delete_download), app);
    // file & folder
    g_signal_connect(menubar->download.delete_file, "activate", G_CALLBACK(on_delete_download_file),
                     app);
    g_signal_connect(menubar->download.open, "activate", G_CALLBACK(on_open_download_file), app);
    g_signal_connect(menubar->download.open_folder, "activate", G_CALLBACK(on_open_download_folder),
                     app);
    // change status
    g_signal_connect(menubar->download.force_start, "activate",
                     G_CALLBACK(on_set_download_force_start), app);
    g_signal_connect(menubar->download.runnable, "activate", G_CALLBACK(on_set_download_runnable),
                     app);
    ...
    g_signal_connect(menubar->download.prioriy.normal, "activate",
                     G_CALLBACK(on_set_download_prioriy), app);
    g_signal_connect(menubar->download.prioriy.low, "activate", G_CALLBACK(on_set_download_prioriy),
                     app);
    // properties
    ...
    // ----------------------------------------------------
    // UgtkHelpMenu
    g_signal_connect(menubar->help.help_online, "activate", G_CALLBACK(on_help_online), app);
    g_signal_connect(menubar->help.documentation, "activate", G_CALLBACK(on_documentation), app);
    g_signal_connect(menubar->help.support_forum, "activate", G_CALLBACK(on_support_forum), app);
    g_signal_connect(menubar->help.submit_feedback, "activate", G_CALLBACK(on_submit_feedback),
                     app);
    g_signal_connect(menubar->help.report_bug, "activate", G_CALLBACK(on_report_bug), app);
    g_signal_connect(menubar->help.keyboard_shortcuts, "activate",
                     G_CALLBACK(on_keyboard_shortcuts), app);
    g_signal_connect(menubar->help.check_updates, "activate", G_CALLBACK(on_check_updates), app);
    g_signal_connect(menubar->help.about_uget, "activate", G_CALLBACK(on_about), app);
}

void
ugtk_menubar_sync_category(UgtkMenubar* menubar, UgtkApp* app, gboolean reset)
{
    UgetNode* cnode;
    UgetCommon* common;
    GtkWidget* menu_item;
    ...
    if (reset) {
        ...
        for (cnode = app->real.children; cnode; cnode = cnode->next) {
            ...
            if (common && common->name)
                menu_item = gtk_menu_item_new_with_label(common->name);
            else
                menu_item = gtk_menu_item_new_with_label("");
            gtk_menu_shell_append(GTK_MENU_SHELL(app->menubar.download.move_to.self), menu_item);
            g_signal_connect(menu_item, "activate", G_CALLBACK(on_move_download), app);
