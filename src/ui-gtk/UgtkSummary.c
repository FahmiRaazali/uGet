...
void
ugtk_summary_init(UgtkSummary* summary, GtkAccelGroup* accel_group)
{
    GtkScrolledWindow* scroll;

    summary->store =
        gtk_list_store_new(UGTK_SUMMARY_N_COLUMN, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
...
void
ugtk_summary_show(UgtkSummary* summary, UgetNode* node)
{
    GtkTreeIter iter;
    gchar* name;
    gchar* value;
    ...
    if (summary->visible.name) {
        ...
        gtk_list_store_set(summary->store, &iter, UGTK_SUMMARY_COLUMN_ICON, "document-symbolic",
                           UGTK_SUMMARY_COLUMN_NAME, name, UGTK_SUMMARY_COLUMN_VALUE, value, -1);
        g_free(name);
    }
    // Summary Folder
    if (summary->visible.folder) {
        ...
        ugtk_summary_store_realloc_next(summary->store, &iter);
        gtk_list_store_set(summary->store, &iter, UGTK_SUMMARY_COLUMN_ICON, "folder-symbolic",
                           UGTK_SUMMARY_COLUMN_NAME, name, UGTK_SUMMARY_COLUMN_VALUE, value, -1);
        g_free(name);
    }
    // Summary Category
    if (summary->visible.category) {
        ...
        if (node->parent) {
            ...
            value = NULL;
        ugtk_summary_store_realloc_next(summary->store, &iter);
        gtk_list_store_set(summary->store, &iter, UGTK_SUMMARY_COLUMN_ICON, "folder-open-symbolic",
                           UGTK_SUMMARY_COLUMN_NAME, name, UGTK_SUMMARY_COLUMN_VALUE, value, -1);
        g_free(name);
    }
    // Summary URL
    if (summary->visible.uri) {
        ...
        ugtk_summary_store_realloc_next(summary->store, &iter);
        gtk_list_store_set(summary->store, &iter, UGTK_SUMMARY_COLUMN_ICON,
                           "network-wired-symbolic", UGTK_SUMMARY_COLUMN_NAME, name,
                           UGTK_SUMMARY_COLUMN_VALUE, value, -1);
        g_free(name);
    }
    // Summary Message
    ...
    if (summary->visible.message) {
        ...
        ugtk_summary_store_realloc_next(summary->store, &iter);
        gtk_list_store_set(summary->store, &iter, UGTK_SUMMARY_COLUMN_ICON, stock,
                           UGTK_SUMMARY_COLUMN_NAME, name, UGTK_SUMMARY_COLUMN_VALUE, value, -1);
    }
...
gchar*
ugtk_summary_get_text_selected(UgtkSummary* summary)
{
    GtkTreeModel* model;
    GtkTreePath* path;
    GtkTreeIter iter;
    ...
    gtk_tree_model_get(model, &iter, UGTK_SUMMARY_COLUMN_NAME, &name, UGTK_SUMMARY_COLUMN_VALUE,
                       &value, -1);
    return g_strconcat(name, " ", value, NULL);
}

gchar*
ugtk_summary_get_text_all(UgtkSummary* summary)
{
    GtkTreeModel* model;
    GtkTreeIter iter;
    gchar* name;
...
ugtk_summary_view_new()
{
    ...
    gtk_tree_view_insert_column_with_attributes(view, UGTK_SUMMARY_COLUMN_ICON, NULL, renderer,
                                                "icon-name", UGTK_SUMMARY_COLUMN_ICON, NULL);
    renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_insert_column_with_attributes(view, UGTK_SUMMARY_COLUMN_NAME, _("Item"), renderer,
                                                "text", UGTK_SUMMARY_COLUMN_NAME, NULL);
    gtk_tree_view_insert_column_with_attributes(view, UGTK_SUMMARY_COLUMN_VALUE, _("Value"),
                                                renderer, "text", UGTK_SUMMARY_COLUMN_VALUE, NULL);
    return view;
}

static void
ugtk_summary_store_realloc_next(GtkListStore* store, GtkTreeIter* iter)
{
    GtkTreeModel* model;
...
static void
ugtk_summary_menu_init(UgtkSummary* summary, GtkAccelGroup* accel_group)
{
    GtkWidget* menu;
    GtkWidget* menu_item;
    ...
    // Copy All
    menu_item = gtk_menu_item_new_with_mnemonic(_("Copy _All"));
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
