...
static void on_select_folder(GtkEntry* entry, GtkEntryIconPosition icon_pos, GdkEvent* event,
                             UgtkDownloadForm* dform);
static void on_select_cookie(GtkEntry* entry, GtkEntryIconPosition icon_pos, GdkEvent* event,
                             UgtkDownloadForm* dform);
static void on_select_post(GtkEntry* entry, GtkEntryIconPosition icon_pos, GdkEvent* event,
                           UgtkDownloadForm* dform);


void
ugtk_download_form_init(UgtkDownloadForm* dform, UgtkProxyForm* proxy, GtkWindow* parent)
{
...
static void
ugtk_download_form_init_page1(UgtkDownloadForm* dform, UgtkProxyForm* proxy)
{
    GtkWidget* widget;
    GtkGrid* top_grid;
    GtkGrid* grid;
    ...
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_uri_entry_changed), dform);
    ...
    widget = gtk_entry_new();
    gtk_entry_set_activates_default(GTK_ENTRY(widget), TRUE);
    gtk_entry_set_placeholder_text(GTK_ENTRY(widget), _("Leave blank to use default filename ..."));
    g_object_set(widget, "margin", 1, "hexpand", TRUE, NULL);
    gtk_grid_attach(top_grid, widget, 1, 2, 2, 1);
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_entry_changed), dform);
    ...
    gtk_entry_set_icon_from_icon_name(GTK_ENTRY(widget), GTK_ENTRY_ICON_SECONDARY,
                                      "folder-symbolic");
    gtk_entry_set_icon_tooltip_text(GTK_ENTRY(widget), GTK_ENTRY_ICON_SECONDARY,
                                    _("Select Folder"));
    g_object_set(dform->folder_combo, "margin", 1, "hexpand", TRUE, NULL);
    gtk_grid_attach(top_grid, dform->folder_combo, 1, 3, 1, 1);
    g_signal_connect(widget, "icon-release", G_CALLBACK(on_select_folder), dform);
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_entry_changed), dform);
    // Folder - label
    ...
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_http_entry_changed), dform);
...
static void
ugtk_download_form_init_page2(UgtkDownloadForm* dform)
{
    GtkWidget* widget;
    GtkGrid* grid;
    ...
    g_signal_connect(widget, "icon-release", G_CALLBACK(on_select_cookie), dform);
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_http_entry_changed), dform);
    ...
    g_signal_connect(widget, "icon-release", G_CALLBACK(on_select_post), dform);
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_http_entry_changed), dform);
    ...
    g_signal_connect(GTK_EDITABLE(widget), "changed", G_CALLBACK(on_spin_changed), dform);
...
void
ugtk_download_form_get(UgtkDownloadForm* dform, UgInfo* node_info)
{
    UgUri uuri;
    const gchar* text;
    gint number;
...
void
ugtk_download_form_set(UgtkDownloadForm* dform, UgInfo* node_info, gboolean keep_changed)
{
    UgetRelation* relation;
    UgetCommon* common;
    UgetHttp* http;
    ...
    if (keep_changed == FALSE || dform->changed.folder == FALSE) {
        g_signal_handlers_block_by_func(GTK_EDITABLE(dform->folder_entry), on_entry_changed, dform);
        gtk_entry_set_text((GtkEntry*) dform->folder_entry,
                           (common && common->folder) ? common->folder : "");
        g_signal_handlers_unblock_by_func(GTK_EDITABLE(dform->folder_entry), on_entry_changed,
                                          dform);
    }
...
void
ugtk_download_form_set_multiple(UgtkDownloadForm* dform, gboolean multiple_mode)
{
...
void
ugtk_download_form_set_folders(UgtkDownloadForm* dform, UgtkSetting* setting)
{
    ...
    g_signal_handlers_unblock_by_func(GTK_EDITABLE(dform->folder_entry), on_entry_changed, dform);
    dform->changed.enable = TRUE;
}

void
ugtk_download_form_get_folders(UgtkDownloadForm* dform, UgtkSetting* setting)
{
    const gchar* current;
...
void
ugtk_download_form_complete_entry(UgtkDownloadForm* dform)
{
    const gchar* text;
    //	gchar*    temp;
    UgUri upart;
...
// signal handler
static void
on_spin_changed(GtkEditable* editable, UgtkDownloadForm* dform)
{
...
static void
on_entry_changed(GtkEditable* editable, UgtkDownloadForm* dform)
{
    if (dform->changed.enable) {
...
static void
on_uri_entry_changed(GtkEditable* editable, UgtkDownloadForm* dform)
{
    if (dform->changed.enable) {
...
static void
on_http_entry_changed(GtkEditable* editable, UgtkDownloadForm* dform)
{
    if (dform->changed.enable) {
...
static void
on_select_folder_response(GtkDialog* chooser, gint response, UgtkDownloadForm* dform)
{
    gchar* file;
    gchar* path;
...
static void
on_select_folder(GtkEntry* entry, GtkEntryIconPosition icon_pos, GdkEvent* event,
                 UgtkDownloadForm* dform)
{
    GtkWidget* chooser;
    gchar* path;
    ...
    chooser = gtk_file_chooser_dialog_new(title, dform->parent,
                                          GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("Cancel"),
                                          GTK_RESPONSE_CANCEL, _("OK"), GTK_RESPONSE_OK, NULL);
    g_free(title);
    ...
    g_signal_connect(chooser, "response", G_CALLBACK(on_select_folder_response), dform);
...
static void
on_select_cookie_response(GtkDialog* chooser, gint response, UgtkDownloadForm* dform)
{
    gchar* file;
    gchar* path;
...
static void
on_select_cookie(GtkEntry* entry, GtkEntryIconPosition icon_pos, GdkEvent* event,
                 UgtkDownloadForm* dform)
{
    GtkWidget* chooser;
    gchar* path;
    ...
    chooser =
        gtk_file_chooser_dialog_new(title, dform->parent, GTK_FILE_CHOOSER_ACTION_OPEN, _("Cancel"),
                                    GTK_RESPONSE_CANCEL, _("OK"), GTK_RESPONSE_OK, NULL);
    g_free(title);
    gtk_window_set_transient_for((GtkWindow*) chooser, dform->parent);
    gtk_window_set_destroy_with_parent((GtkWindow*) chooser, TRUE);
    ...
    g_signal_connect(chooser, "response", G_CALLBACK(on_select_cookie_response), dform);
...
static void
on_select_post_response(GtkDialog* chooser, gint response, UgtkDownloadForm* dform)
{
    gchar* file;
    gchar* path;
...
static void
on_select_post(GtkEntry* entry, GtkEntryIconPosition icon_pos, GdkEvent* event,
               UgtkDownloadForm* dform)
{
    GtkWidget* chooser;
    gchar* path;
    ...
    g_signal_connect(chooser, "response", G_CALLBACK(on_select_post_response), dform);
