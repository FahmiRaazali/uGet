static const char uget_license[] = {
    " Copyright (C) 2005-2025 by C.H. Huang"
    "\n"
...
// static data
static const gchar *uget_authors[] = {
    "C.H. Huang  (\xE9\xBB\x83\xE6\xAD\xA3\xE9\x9B\x84)", NULL};
static const gchar *uget_artists[] = {"Michael Tunnell (visuex.com)", NULL};
static const gchar *uget_comments = N_("Download Manager");
static const gchar *uget_copyright = "Copyright (C) 2005-2025 C.H. Huang";
...
UgtkAboutDialog *ugtk_about_dialog_new(GtkWindow *parent) {
  ...
  // Try standard install path: .../pixmaps/uget/logo.png
  const char *data_dir = ugtk_get_data_dir();
  path = g_build_filename(data_dir, "pixmaps", "uget", "logo.png", NULL);

  if (!g_file_test(path, G_FILE_TEST_EXISTS)) {
    g_free(path);
    // Try source/flat layout: .../pixmaps/logo.png
    path = g_build_filename(data_dir, "pixmaps", "logo.png", NULL);
  }
  ...
  comments = g_strconcat(
      ...
      _("Contributor: Fahmi Raazali (fahmiraazali.com)"), "\n", NULL);

  g_object_set(
      adialog->self,
      //			"logo-icon-name", UGTK_APP_ICON_NAME,
      "logo", adialog->pixbuf, "program-name", UGTK_APP_NAME, "version",
      PACKAGE_VERSION, "comments", comments, "copyright", uget_copyright,
#if defined _WIN32 || defined _WIN64
      "website-label", UGET_URL_WEBSITE,
#else
      "website", UGET_URL_WEBSITE,
#endif
      "license", uget_license, "authors", uget_authors, "artists", uget_artists,
      "translator-credits", gettext(translator_credits), NULL);
...
void ugtk_about_dialog_set_info(UgtkAboutDialog *adialog,
