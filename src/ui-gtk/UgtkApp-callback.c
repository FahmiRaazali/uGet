...
void
ugtk_app_init_callback(UgtkApp* app)
{
    ...
    //	                         g_cclosure_new_swap (G_CALLBACK (on_summary_copy_selected), app,
    // NULL));
    ...
    // node notification
    uget_app_set_notification((UgetApp*) app, app, node_inserted, node_removed,
                              (UgNotifyFunc) node_updated);
}
...
static void
on_download_selection_changed(GtkTreeSelection* selection, UgtkApp* app)
{
    gint n_selected;
...
static void
on_download_cursor_changed(GtkTreeView* view, UgtkApp* app)
{
    GtkWidget* item;
    UgetNode* node;
...
static void
on_category_cursor_changed(GtkTreeView* view, UgtkApp* app)
{
    ...
    if (traveler->state.cursor.pos != traveler->state.cursor.pos_last ||
        ...
        // set sorting column
        ugtk_traveler_set_sorting(traveler, TRUE, app->setting.download_column.sort.nth,
...
// button-press-event
static gboolean
on_button_press_event(GtkTreeView* view, GdkEventButton* event, UgtkApp* app)
{
...
static void
on_summary_copy_selected(GtkWidget* widget, UgtkApp* app)
{
    gchar* text;
...
static void
on_summary_copy_all(GtkWidget* widget, UgtkApp* app)
{
    gchar* text;
...
// key-press-event
static gboolean
on_window_key_press_event(GtkWidget* widget, GdkEventKey* event, UgtkApp* app)
{
    GtkTreeView* focus;
    GtkMenu* menu;
    ...
    gtk_menu_popup_at_widget(menu, widget, GDK_GRAVITY_CENTER, GDK_GRAVITY_NORTH_WEST, NULL);
    return TRUE;
}

// UgtkWindow.self "delete-event"
static gboolean
on_window_delete_event(GtkWidget* widget, GdkEvent* event, UgtkApp* app)
{
...
static gboolean
on_traveler_key_press_event(GtkWidget* widget, GdkEventKey* event, UgtkApp* app)
{
    /*
...
static void
ugtk_window_init_callback(struct UgtkWindow* window, UgtkApp* app)
{
    GtkTreeSelection* selection;

    // UgtkTraveler
    selection = gtk_tree_view_get_selection(app->traveler.download.view);
    g_signal_connect(selection, "changed", G_CALLBACK(on_download_selection_changed), app);
    ...
    g_signal_connect(app->traveler.download.view, "button-press-event",
                     G_CALLBACK(on_button_press_event), app);
    g_signal_connect(app->summary.view, "button-press-event", G_CALLBACK(on_button_press_event),
                     app);

    // UgtkSummary.menu signal handlers
    g_signal_connect(app->summary.menu.copy, "activate", G_CALLBACK(on_summary_copy_selected), app);
    g_signal_connect(app->summary.menu.copy_all, "activate", G_CALLBACK(on_summary_copy_all), app);

    // UgtkWindow.self signal handlers
    g_signal_connect(window->self, "key-press-event", G_CALLBACK(on_window_key_press_event), app);
    g_signal_connect(window->self, "delete-event", G_CALLBACK(on_window_delete_event), app);
    g_signal_connect_swapped(window->self, "destroy", G_CALLBACK(ugtk_app_quit), app);
...
static void
ugtk_toolbar_init_callback(struct UgtkToolbar* toolbar, UgtkApp* app)
{
    // create new
    g_signal_connect(toolbar->create, "clicked", G_CALLBACK(on_create_download), app);
    g_signal_connect(toolbar->create_download, "activate", G_CALLBACK(on_create_download), app);
    ...
    // save
    g_signal_connect_swapped(toolbar->save, "clicked", G_CALLBACK(ugtk_app_save), app);
    // change status
    g_signal_connect(toolbar->runnable, "clicked", G_CALLBACK(on_set_download_runnable), app);
    g_signal_connect_swapped(toolbar->pause, "clicked", G_CALLBACK(ugtk_app_pause_download), app);
    // change data
    g_signal_connect_swapped(toolbar->properties, "clicked", G_CALLBACK(ugtk_app_edit_download),
                             app);
    // move
    g_signal_connect_swapped(toolbar->move_up, "clicked", G_CALLBACK(ugtk_app_move_download_up),
                             app);
    g_signal_connect_swapped(toolbar->move_down, "clicked", G_CALLBACK(ugtk_app_move_download_down),
                             app);
    g_signal_connect_swapped(toolbar->move_top, "clicked", G_CALLBACK(ugtk_app_move_download_top),
                             app);
...
static void
node_inserted(UgetNode* node, UgetNode* sibling, UgetNode* child)
{
    GtkTreePath* path;
    GtkTreeIter iter;
    UgtkApp* app;
    ...
    if (node == (UgetNode*) app->traveler.category.model->root) {
        // category inserted
        path = gtk_tree_path_new_from_indices(uget_node_child_position(node, child) + 1, -1);
        iter.stamp = app->traveler.category.model->stamp;
        iter.user_data = child;
        gtk_tree_model_row_inserted(GTK_TREE_MODEL(app->traveler.category.model), path, &iter);
        gtk_tree_path_free(path);
        // sync UgtkMenubar.download.move_to
        ugtk_menubar_sync_category(&app->menubar, app, TRUE);
    } else if (node == (UgetNode*) app->traveler.download.model->root) {
        // download inserted
        path = gtk_tree_path_new_from_indices(uget_node_child_position(node, child), -1);
        iter.stamp = app->traveler.download.model->stamp;
        iter.user_data = child;
        gtk_tree_model_row_inserted(GTK_TREE_MODEL(app->traveler.download.model), path, &iter);
        gtk_tree_path_free(path);
    }
}

static void
node_removed(UgetNode* node, UgetNode* sibling, UgetNode* child)
{
    GtkTreePath* path;
    UgtkApp* app;
    int pos;
    ...
    if (node == (UgetNode*) app->traveler.category.model->root) {
        ...
        gtk_tree_model_row_deleted(GTK_TREE_MODEL(app->traveler.category.model), path);
        gtk_tree_path_free(path);
        // sync UgtkMenubar.download.move_to
        ugtk_menubar_sync_category(&app->menubar, app, TRUE);
    } else if (node == (UgetNode*) app->traveler.download.model->root) {
        ...
        gtk_tree_model_row_deleted(GTK_TREE_MODEL(app->traveler.download.model), path);
        gtk_tree_path_free(path);
    }
}

static void
node_updated(UgetNode* child)
{
    GtkTreePath* path;
    GtkTreeIter iter;
    UgetNode* node;
    ...
    if (node == (UgetNode*) app->traveler.category.model->root) {
        ...
        iter.stamp = app->traveler.category.model->stamp;
        iter.user_data = child;
        gtk_tree_model_row_changed(GTK_TREE_MODEL(app->traveler.category.model), path, &iter);
        gtk_tree_path_free(path);
        // sync UgtkMenubar.download.move_to
        ugtk_menubar_sync_category(&app->menubar, app, TRUE);
    } else if (node == (UgetNode*) app->traveler.download.model->root) {
        ...
        iter.stamp = app->traveler.download.model->stamp;
        iter.user_data = child;
        gtk_tree_model_row_changed(GTK_TREE_MODEL(app->traveler.download.model), path, &iter);
        gtk_tree_path_free(path);
    }
}
