...
void ugtk_tray_icon_init(UgtkTrayIcon *trayicon) {
  ...
  image =
      gtk_image_new_from_icon_name("document-new-symbolic", GTK_ICON_SIZE_MENU);
  ...
  // New Clipboard batch
  menu_item =
      gtk_image_menu_item_new_with_mnemonic(_("New Clipboard _batch..."));
  image =
      gtk_image_new_from_icon_name("edit-paste-symbolic", GTK_ICON_SIZE_MENU);
  gtk_image_menu_item_set_image((GtkImageMenuItem *)menu_item, image);
  gtk_menu_shell_append((GtkMenuShell *)menu, menu_item);
  ...
  menu_item =
      gtk_check_menu_item_new_with_mnemonic(_("Clipboard works quietly"));
  gtk_menu_shell_append((GtkMenuShell *)menu, menu_item);
  trayicon->menu.clipboard_quiet = menu_item;

  menu_item =
      gtk_check_menu_item_new_with_mnemonic(_("Command-line works quietly"));
  gtk_menu_shell_append((GtkMenuShell *)menu, menu_item);
  ...
  menu_item = gtk_check_menu_item_new_with_mnemonic(
      _("Apply recent download settings"));
  gtk_menu_shell_append((GtkMenuShell *)menu, menu_item);
  ...
  image = gtk_image_new_from_icon_name("document-properties-symbolic",
                                       GTK_ICON_SIZE_MENU);
  gtk_image_menu_item_set_image((GtkImageMenuItem *)menu_item, image);
  gtk_menu_shell_append((GtkMenuShell *)menu, menu_item);
  ...
  file_name = g_build_filename(data_dir, "icons", "hicolor", "16x16", "apps",
                               "uget-icon.png", NULL);

  if (!g_file_test(file_name, G_FILE_TEST_IS_REGULAR)) {
    g_free(file_name);
    // Try source directory layout
    file_name = g_build_filename(data_dir, "pixmaps", "icons", "hicolor",
                                 "16x16", "apps", "uget-icon.png", NULL);
  }
  ...
  g_free(file_name);
#ifdef HAVE_APP_INDICATOR
  trayicon->indicator = app_indicator_new(
      "uget-gtk", icon_name, APP_INDICATOR_CATEGORY_APPLICATION_STATUS);
...
void ugtk_tray_icon_set_info(UgtkTrayIcon *trayicon, guint n_active,
                             gint64 down_speed, gint64 up_speed) {
  gchar *string;
  ...
  if (trayicon->state != current_state) {
    ...
    if (trayicon->indicator) {
      ...
      if (app_indicator_get_status(trayicon->indicator) !=
  ...
  string =
      g_strdup_printf(UGTK_APP_NAME " " PACKAGE_VERSION "\n"
                                    "%u %s"
                                    "\n"
                                    "\xE2\x86\x93 %s" // "↓"
                                    " , "
  ...
  if (trayicon->indicator) {
    ...
    //		g_object_set (trayicon->indicator, "attention-icon-desc",
    // string, NULL);
  } else
#endif // HAVE_APP_INDICATOR
...
//		gtk_window_deiconify (app->window.self);
    ...
    gtk_window_move(app->window.self, app->setting.window.x,
                    app->setting.window.y);
    gtk_window_resize(app->window.self, app->setting.window.width,
                      app->setting.window.height);
  }
  // clear error status
...
#if defined _WIN32 || defined _WIN64
  gtk_menu_popup((GtkMenu *)trayicon->menu.self, NULL, NULL, NULL, NULL, button,
                 activate_time);
#else
  gtk_menu_popup((GtkMenu *)trayicon->menu.self, NULL, NULL,
...
static gboolean tray_menu_leave_enter(GtkWidget *menu, GdkEventCrossing *event,
                                      gpointer data) {
  ...
  if (event->type == GDK_LEAVE_NOTIFY &&
    ...
    if (tray_menu_timer == 0) {
      tray_menu_timer =
          g_timeout_add(500, (GSourceFunc)tray_menu_timeout, menu);
    }
...
void ugtk_trayicon_init_callback(struct UgtkTrayIcon *trayicon, UgtkApp *app) {
  g_signal_connect(trayicon->self, "activate", G_CALLBACK(on_trayicon_activate),
                   app);
