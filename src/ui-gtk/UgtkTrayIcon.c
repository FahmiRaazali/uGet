...
void
ugtk_tray_icon_init(UgtkTrayIcon* trayicon)
{
    GtkWidget* menu;
    GtkWidget* menu_item;
    gchar* icon_name;
    ...
    menu = gtk_menu_new();
    // New Download
    menu_item = gtk_menu_item_new_with_mnemonic(_("New _Download..."));
    gtk_menu_shell_append((GtkMenuShell*) menu, menu_item);
    trayicon->menu.create_download = menu_item;

    // New Clipboard batch
    menu_item = gtk_menu_item_new_with_mnemonic(_("New Clipboard _batch..."));
    gtk_menu_shell_append((GtkMenuShell*) menu, menu_item);
    ...
    // New Torrent
    menu_item = gtk_menu_item_new_with_mnemonic(_("New Torrent..."));
    gtk_menu_shell_append((GtkMenuShell*) menu, menu_item);
    trayicon->menu.create_torrent = menu_item;

    // New Metalink
    menu_item = gtk_menu_item_new_with_mnemonic(_("New Metalink..."));
    gtk_menu_shell_append((GtkMenuShell*) menu, menu_item);
    ...
    // About
    menu_item = gtk_menu_item_new_with_mnemonic(_("About"));
    gtk_menu_shell_append((GtkMenuShell*) menu, menu_item);
    ...
    // decide tray icon
    const char* data_dir = ugtk_get_data_dir();
    file_name =
        g_build_filename(data_dir, "icons", "hicolor", "16x16", "apps", "uget-icon.png", NULL);

    if (!g_file_test(file_name, G_FILE_TEST_IS_REGULAR)) {
        g_free(file_name);
        // Try source directory layout
        file_name = g_build_filename(data_dir, "pixmaps", "icons", "hicolor", "16x16", "apps",
                                     "uget-icon.png", NULL);
    }
...
#ifdef HAVE_APP_INDICATOR
    trayicon->indicator =
        app_indicator_new("uget-gtk", icon_name, APP_INDICATOR_CATEGORY_APPLICATION_STATUS);
    ...
    if (trayicon->indicator) {
        ...
        app_indicator_set_attention_icon(trayicon->indicator, UGTK_TRAY_ICON_ACTIVE_NAME);
...
void
ugtk_tray_icon_set_info(UgtkTrayIcon* trayicon, guint n_active, gint64 down_speed, gint64 up_speed)
{
    gchar* string;
        ...
        if (trayicon->indicator) {
            ...
            if (app_indicator_get_status(trayicon->indicator) != APP_INDICATOR_STATUS_PASSIVE) {
                if (current_state == UGTK_TRAY_ICON_STATE_NORMAL) {
                    ...
                    app_indicator_set_status(trayicon->indicator, APP_INDICATOR_STATUS_ATTENTION);
                }
            }
        } else
#endif // HAVE_APP_INDICATOR
    ...
    string = g_strdup_printf(UGTK_APP_NAME " " PACKAGE_VERSION "\n"
                                           "%u %s"
                                           "\n"
                                           "\xE2\x86\x93 %s" // "↓"
                                           " , "
                                           "\xE2\x86\x91 %s", // "↑"
                             n_active, _("tasks"), string_down_speed, string_up_speed);
#ifdef HAVE_APP_INDICATOR
    if (trayicon->indicator) {
        // indicator doesn't support tooltip text
    } else
#endif // HAVE_APP_INDICATOR
    {
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS
...
void
ugtk_tray_icon_set_visible(UgtkTrayIcon* trayicon, gboolean visible)
{
#ifdef HAVE_APP_INDICATOR
    if (trayicon->indicator) {
        if (visible)
...
void
ugtk_tray_icon_use_indicator(UgtkTrayIcon* trayicon, gboolean enable)
{
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
static void
on_trayicon_activate(GtkStatusIcon* status_icon, UgtkApp* app)
{
    gint x, y;
    ...
    if (gtk_widget_get_visible((GtkWidget*) app->window.self) == TRUE) {
        ...
        gtk_window_get_size(app->window.self, &app->setting.window.width,
                            &app->setting.window.height);
...
//		gtk_window_deiconify (app->window.self);
        ...
        gtk_window_move(app->window.self, app->setting.window.x, app->setting.window.y);
        gtk_window_resize(app->window.self, app->setting.window.width, app->setting.window.height);
    }
    // clear error status
...
static void
on_trayicon_popup_menu(GtkStatusIcon* status_icon, guint button, guint activate_time,
                       UgtkTrayIcon* trayicon)
{
    gtk_menu_set_screen((GtkMenu*) trayicon->menu.self, gtk_status_icon_get_screen(status_icon));
#if defined _WIN32 || defined _WIN64
    gtk_menu_popup((GtkMenu*) trayicon->menu.self, NULL, NULL, NULL, NULL, button, activate_time);
#else
    gtk_menu_popup((GtkMenu*) trayicon->menu.self, NULL, NULL, gtk_status_icon_position_menu,
...
#if defined _WIN32 || defined _WIN64
static gboolean
tray_menu_timeout(GtkMenu* menu)
{
...
static gboolean
tray_menu_leave_enter(GtkWidget* menu, GdkEventCrossing* event, gpointer data)
{
    ...
    if (event->type == GDK_LEAVE_NOTIFY &&
        ...
        if (tray_menu_timer == 0) {
            tray_menu_timer = g_timeout_add(500, (GSourceFunc) tray_menu_timeout, menu);
        }
...
on_offline_mode(GtkWidget* widget, UgtkApp* app)
{
    ...
    // file menu
    offline = gtk_check_menu_item_get_active((GtkCheckMenuItem*) app->menubar.file.offline_mode);
    if (offline != app->setting.offline_mode) {
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.file.offline_mode,
...
#ifdef HAVE_APP_INDICATOR
static void
on_trayicon_show_window(GtkWidget* widget, UgtkApp* app)
{
...
void
ugtk_trayicon_init_callback(struct UgtkTrayIcon* trayicon, UgtkApp* app)
{
    g_signal_connect(trayicon->self, "activate", G_CALLBACK(on_trayicon_activate), app);
    g_signal_connect(trayicon->self, "popup-menu", G_CALLBACK(on_trayicon_popup_menu), trayicon);

#if defined _WIN32 || defined _WIN64
    g_signal_connect(trayicon->menu.self, "leave-notify-event", G_CALLBACK(tray_menu_leave_enter),
                     NULL);
    g_signal_connect(trayicon->menu.self, "enter-notify-event", G_CALLBACK(tray_menu_leave_enter),
                     NULL);
#endif
    g_signal_connect(trayicon->menu.create_download, "activate", G_CALLBACK(on_create_download),
                     app);
    ...
    g_signal_connect(trayicon->menu.clipboard_monitor, "activate", G_CALLBACK(on_clipboard_monitor),
                     app);
    g_signal_connect(trayicon->menu.clipboard_quiet, "activate", G_CALLBACK(on_clipboard_quiet),
                     app);
    g_signal_connect(trayicon->menu.commandline_quiet, "activate", G_CALLBACK(on_commandline_quiet),
                     app);
    g_signal_connect(trayicon->menu.skip_existing, "activate", G_CALLBACK(on_skip_existing), app);
    g_signal_connect(trayicon->menu.apply_recent, "activate", G_CALLBACK(on_apply_recent), app);

    g_signal_connect(trayicon->menu.settings, "activate", G_CALLBACK(on_config_settings), app);
    g_signal_connect(trayicon->menu.offline_mode, "toggled", G_CALLBACK(on_offline_mode), app);
    g_signal_connect(trayicon->menu.about, "activate", G_CALLBACK(on_about), app);
    g_signal_connect_swapped(trayicon->menu.quit, "activate", G_CALLBACK(ugtk_app_decide_to_quit),
                             app);

#ifdef HAVE_APP_INDICATOR
    g_signal_connect(trayicon->menu.show_window, "activate", G_CALLBACK(on_trayicon_show_window),
                     app);
#endif // HAVE_APP_INDICATOR
}
