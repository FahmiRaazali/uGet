...
static gboolean motion_notify_event(GtkWidget* tv_widget, GdkEventMotion* event,
                                    UgtkBanner* banner);

static gboolean event_after(GtkWidget* text_view, GdkEvent* ev, UgtkBanner* banner);

static GtkWidget* create_x_button(UgtkBanner* banner);

void
ugtk_banner_init(struct UgtkBanner* banner)
{
    banner->self = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    banner->buffer = gtk_text_buffer_new(NULL);
    banner->tag_link =
        gtk_text_buffer_create_tag(banner->buffer, NULL, "underline", PANGO_UNDERLINE_SINGLE, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(banner->self), GTK_WIDGET(banner->text_view), TRUE, TRUE, 0);

    g_signal_connect(banner->text_view, "event-after", G_CALLBACK(event_after), banner);
    g_signal_connect(banner->text_view, "motion-notify-event", G_CALLBACK(motion_notify_event),
                     banner);
    // close button
    gtk_box_pack_end(GTK_BOX(banner->self), create_x_button(banner), FALSE, FALSE, 0);
...
int
ugtk_banner_show_rss(UgtkBanner* banner, UgetRss* urss)
{
...
void
ugtk_banner_show(UgtkBanner* banner, const char* title, const char* url)
{
    GtkTextIter iter;
    ...
    if (url == NULL) {
        ...
        gtk_text_buffer_insert_with_tags(banner->buffer, &iter, title, -1, banner->tag_link, NULL);
    }
    gtk_widget_show(banner->self);
}

void
ugtk_banner_show_donation(UgtkBanner* banner)
{
    GtkTextIter iter;
    ...
    gtk_text_buffer_insert(banner->buffer, &iter,
                           ...
                           -1);
    gtk_text_buffer_insert_with_tags(banner->buffer, &iter, _("HERE"), -1, banner->tag_link, NULL);
...
void
ugtk_banner_show_survey(UgtkBanner* banner)
{
    GtkTextIter iter;
    ...
    gtk_text_buffer_insert_with_tags(banner->buffer, &iter, _("click here to take survey"), -1,
                                     banner->tag_link, NULL);
...
motion_notify_event(GtkWidget* tv_widget, GdkEventMotion* event, UgtkBanner* banner)
{
    ...
    gtk_text_view_window_to_buffer_coords(text_view, GTK_TEXT_WINDOW_WIDGET, (gint) event->x,
                                          (gint) event->y, &x, &y);
...
static GtkWidget*
create_x_button(UgtkBanner* banner)
{
    GtkWidget* event_box;
    GtkWidget* label;
    ...
    g_signal_connect(event_box, "button-release-event", G_CALLBACK(on_x_button_release), banner);
    return event_box;
}
