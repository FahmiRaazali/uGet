...
void
ugtk_app_init(UgtkApp* app, UgetRpc* rpc)
{
    char* dir;
...
ugtk_app_load(UgtkApp* app)
{
    ...
    if (counts == FALSE)
        ...
        ug_array_alloc(&app->setting.scheduler.state, 7 * 24 - app->setting.scheduler.state.length);
    }
...
void
ugtk_app_get_window_setting(UgtkApp* app, UgtkSetting* setting)
{
    ...
    if (gtk_widget_get_visible(GTK_WIDGET(app->window.self)) == TRUE) {
        ...
        if (setting->window.maximized == FALSE) {
            ...
            gtk_window_get_size(app->window.self, &setting->window.width, &setting->window.height);
...
void
ugtk_app_set_window_setting(UgtkApp* app, UgtkSetting* setting)
{
    // Get screen dimensions using modern GdkDisplay/GdkMonitor API
    ...
    if (monitor) {
        ...
        // Fallback to reasonable defaults if no monitor found
        screen_width = 1920;
        screen_height = 1080;
    }
    ...
    if (setting->window.width > 0 && setting->window.height > 0 &&
        setting->window.x < screen_width && setting->window.y < screen_height &&
        setting->window.x + setting->window.width > 0 &&
        setting->window.y + setting->window.height > 0) {
        gtk_window_move(app->window.self, setting->window.x, setting->window.y);
        gtk_window_resize(app->window.self, setting->window.width, setting->window.height);
    }
    if (setting->window.maximized)
        gtk_window_maximize(app->window.self);
    // GtkPaned position
    if (setting->window.paned_position_h > 0) {
        gtk_paned_set_position(app->window.hpaned, setting->window.paned_position_h);
    }
    if (setting->window.paned_position_v > 0) {
        if (setting->window.paned_position_v > 100) // for uGet < 2.0.4
            gtk_paned_set_position(app->window.vpaned, setting->window.paned_position_v);
    }
    // set visible widgets
    gtk_widget_set_visible(app->toolbar.self, setting->window.toolbar);
    gtk_widget_set_visible((GtkWidget*) app->statusbar.self, setting->window.statusbar);
    gtk_widget_set_visible(gtk_paned_get_child1(app->window.hpaned), setting->window.category);
    gtk_widget_set_visible(app->summary.self, setting->window.summary);
    gtk_widget_set_visible(app->banner.self, setting->window.banner);
    // Summary
    ...
    ugtk_traveler_select_category(&app->traveler, setting->window.nth_category,
...
void
ugtk_app_get_column_setting(UgtkApp* app, UgtkSetting* setting)
{
    GtkTreeViewColumn* column;
    int width;

    // state
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_STATE);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.state = width;
    // name
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_NAME);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.name = width;
    // complete
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_COMPLETE);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.complete = width;
    // total
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_TOTAL);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.total = width;
    // percent
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_PERCENT);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.percent = width;
    // elapsed
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_ELAPSED);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.elapsed = width;
    // left
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_LEFT);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.left = width;
    // speed
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_SPEED);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.speed = width;
    // upload_speed
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_UPLOAD_SPEED);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.upload_speed = width;
    // uploaded
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_UPLOADED);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.uploaded = width;
    // ratio
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_RATIO);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.ratio = width;
    // retry
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_RETRY);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.retry = width;
    // category
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_CATEGORY);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.category = width;
    // uri
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_URI);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.uri = width;
    // added_on
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_ADDED_ON);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.added_on = width;
    // completed_on
    column = gtk_tree_view_get_column(app->traveler.download.view, UGTK_NODE_COLUMN_COMPLETED_ON);
    width = gtk_tree_view_column_get_width(column);
    setting->download_column.width.completed_on = width;
}

void
ugtk_app_set_column_setting(UgtkApp* app, UgtkSetting* setting)
{
    GtkTreeViewColumn* column;
    int width;
...
void
ugtk_app_set_plugin_setting(UgtkApp* app, UgtkSetting* setting)
{
    ...
    uget_plugin_agent_global_set(UGET_PLUGIN_AGENT_GLOBAL_PLUGIN, (void*) default_plugin);
    // set media plug-in
...
void
ugtk_app_set_other_setting(UgtkApp* app, UgtkSetting* setting)
{
    ...
    // global speed limit
    uget_task_set_speed(&app->task, setting->bandwidth.normal.download * 1024,
                        setting->bandwidth.normal.upload * 1024);
}

void
ugtk_app_set_menu_setting(UgtkApp* app, UgtkSetting* setting)
{
    // ----------------------------------------------------
    // UgtkEditMenu
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.clipboard_monitor,
                                   setting->clipboard.monitor);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.clipboard_quiet,
                                   setting->clipboard.quiet);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.commandline_quiet,
                                   setting->commandline.quiet);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.skip_existing,
                                   setting->ui.skip_existing);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.apply_recent,
                                   setting->ui.apply_recent);

    switch (app->setting.completion.action) {
    default:
    case 0:
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.disable,
                                       TRUE);
        break;

    case 1:
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.hibernate,
                                       TRUE);
        break;

    case 2:
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.suspend,
                                       TRUE);
        break;

    case 3:
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.shutdown,
                                       TRUE);
        break;

    case 4:
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.reboot,
                                       TRUE);
        break;

    case 5:
        gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.custom,
                                       TRUE);
        break;
    }
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.edit.completion.remember,
                                   setting->completion.remember);
    // ----------------------------------------------------
    // UgtkViewMenu
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.toolbar,
                                   setting->window.toolbar);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.statusbar,
                                   setting->window.statusbar);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.category,
                                   setting->window.category);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.summary,
                                   setting->window.summary);
    // summary items
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.summary_items.name,
                                   setting->summary.name);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.summary_items.folder,
                                   setting->summary.folder);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.summary_items.category,
                                   setting->summary.category);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.summary_items.uri,
                                   setting->summary.uri);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.summary_items.message,
                                   setting->summary.message);
    // download column
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.complete,
                                   setting->download_column.complete);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.total,
                                   setting->download_column.total);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.percent,
                                   setting->download_column.percent);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.elapsed,
                                   setting->download_column.elapsed);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.left,
                                   setting->download_column.left);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.speed,
                                   setting->download_column.speed);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.upload_speed,
                                   setting->download_column.upload_speed);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.uploaded,
                                   setting->download_column.uploaded);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.ratio,
                                   setting->download_column.ratio);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.retry,
                                   setting->download_column.retry);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.category,
                                   setting->download_column.category);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.uri,
                                   setting->download_column.uri);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.added_on,
                                   setting->download_column.added_on);
    gtk_check_menu_item_set_active((GtkCheckMenuItem*) app->menubar.view.columns.completed_on,
                                   setting->download_column.completed_on);
}

void
ugtk_app_set_ui_setting(UgtkApp* app, UgtkSetting* setting)
{
    ...
    // state list
    ugtk_node_view_use_large_icon(app->traveler.state.view, setting->ui.large_icon,
                                  setting->download_column.width.state);
    // category list
    ugtk_node_view_use_large_icon(app->traveler.category.view, setting->ui.large_icon,
                                  setting->download_column.width.state);
    // download list
    ugtk_node_view_use_large_icon(app->traveler.download.view, setting->ui.large_icon,
                                  setting->download_column.width.state);
    // summary
    ugtk_node_view_use_large_icon(app->summary.view, setting->ui.large_icon,
                                  setting->download_column.width.state);
}

// decide sensitive for menu, toolbar
void
ugtk_app_decide_download_sensitive(UgtkApp* app)
{
...
// decide sensitive for menu, toolbar
void
ugtk_app_decide_category_sensitive(UgtkApp* app)
{
    static gboolean sensitive_last = TRUE;
    gboolean sensitive;
...
void
ugtk_app_decide_trayicon_visible(UgtkApp* app)
{
    gboolean visible;
...
void
ugtk_app_decide_to_quit(UgtkApp* app)
{
...
void
ugtk_app_create_category(UgtkApp* app)
{
    ...
    ug_free(common->name);
    common->name =
        ug_strdup_printf("%s%s", _("Copy - "), (common_src->name) ? common_src->name : NULL);
...
void
ugtk_app_create_download(UgtkApp* app, const char* sub_title, const char* uri)
{
    UgtkNodeDialog* ndialog;
    UgUri uuri;
    UgetNode* cnode;
...
void
ugtk_app_delete_category(UgtkApp* app)
{
    UgetNode* cnode;
    int pos;
...
void
ugtk_app_delete_download(UgtkApp* app, gboolean delete_files)
{
    UgetNode* cursor;
    UgetNode* node;
    GList* link;
    ...
    dev_pointer =
        gdk_seat_get_pointer(gdk_display_get_default_seat(gdk_window_get_display(gdk_win)));
...
void
ugtk_app_edit_category(UgtkApp* app)
{
    UgtkNodeDialog* ndialog;
    UgetNode* node;
    gchar* title;
...
void
ugtk_app_edit_download(UgtkApp* app)
{
    UgtkNodeDialog* ndialog;
    UgetNode* node;
    gchar* title;
...
void
ugtk_app_queue_download(UgtkApp* app, gboolean keep_active)
{
...
void
ugtk_app_pause_download(UgtkApp* app)
{
    UgetNode* node;
    UgetNode* cursor;
    GList* list;
...
void
ugtk_app_switch_download_state(UgtkApp* app)
{
    UgetRelation* relation;
    UgetNode* node;
    UgetNode* cursor;
...
void
ugtk_app_move_download_up(UgtkApp* app)
{
...
void
ugtk_app_move_download_down(UgtkApp* app)
{
...
void
ugtk_app_move_download_top(UgtkApp* app)
{
...
void
ugtk_app_move_download_bottom(UgtkApp* app)
{
...
void
ugtk_app_move_download_to(UgtkApp* app, UgetNode* cnode)
{
    UgetNode* node;
    GList* link;
    GList* list = NULL;
...
static GtkWidget*
create_file_chooser(GtkWindow* parent, GtkFileChooserAction action, const gchar* title,
                    const gchar* filter_name, const gchar* mine_type)
{
    GtkWidget* dialog;
    GtkFileFilter* filter;

    dialog = gtk_file_chooser_dialog_new(title, parent, action, _("Cancel"), GTK_RESPONSE_CANCEL,
...
static void
on_create_torrent_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    gchar* file;
...
static void
on_create_metalink_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    gchar* file;
...
void
ugtk_app_create_torrent(UgtkApp* app)
{
    GtkWidget* dialog;
    gchar* title;

    title = g_strconcat(UGTK_APP_NAME " - ", _("Open Torrent file"), NULL);
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_OPEN, title,
                                 _("Torrent file (*.torrent)"), "application/x-bittorrent");
    g_free(title);
    g_signal_connect(dialog, "response", G_CALLBACK(on_create_torrent_response), app);
    gtk_widget_show(dialog);
}

void
ugtk_app_create_metalink(UgtkApp* app)
{
    GtkFileFilter* filter;
    GtkWidget* dialog;
    gchar* title;
    ...
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_OPEN, title, NULL, NULL);
    g_free(title);
    ...
    gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(dialog), filter);
    g_signal_connect(dialog, "response", G_CALLBACK(on_create_metalink_response), app);
    gtk_widget_show(dialog);
}
...
static void
on_save_category_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    UgetNode* cnode;
    gchar* file;
...
static void
on_load_category_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    gchar* file;
...
void
ugtk_app_save_category(UgtkApp* app)
{
    GtkWidget* dialog;
    gchar* title;
    ...
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_SAVE, title, NULL, NULL);
    g_free(title);

    g_signal_connect(dialog, "response", G_CALLBACK(on_save_category_response), app);
    gtk_widget_show(dialog);
}

void
ugtk_app_load_category(UgtkApp* app)
{
    GtkWidget* dialog;
    gchar* title;
    ...
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_OPEN, title,
                                 _("JSON file (*.json)"), "application/json");
    g_free(title);

    g_signal_connect(dialog, "response", G_CALLBACK(on_load_category_response), app);
    gtk_widget_show(dialog);
}

static void
on_import_html_file_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    UgHtmlFilter* filter;
    ...
    // UgtkBatchDialog
    bdialog = ugtk_batch_dialog_new(gtk_window_get_title((GtkWindow*) dialog), app);
...
static void
on_import_text_file_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    UgtkBatchDialog* bdialog;
    UgtkSelectorPage* page;
    UgetNode* cnode;
...
static void
on_export_text_file_response(GtkWidget* dialog, gint response, UgtkApp* app)
{
    GIOChannel* channel;
    UgetCommon* common;
    UgetNode* node;
...
void
ugtk_app_import_html_file(UgtkApp* app)
{
    GtkWidget* dialog;
    gchar* title;
    ...
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_OPEN, title,
                                 _("HTML file (*.htm, *.html)"), "text/html");
    g_free(title);
    g_signal_connect(dialog, "response", G_CALLBACK(on_import_html_file_response), app);
    gtk_widget_show(dialog);
}

void
ugtk_app_import_text_file(UgtkApp* app)
{
    GtkWidget* dialog;
    gchar* title;
    ...
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_OPEN, title,
                                 _("Plain text file"), "text/plain");
    g_free(title);
    g_signal_connect(dialog, "response", G_CALLBACK(on_import_text_file_response), app);
    gtk_widget_show(dialog);
}

void
ugtk_app_export_text_file(UgtkApp* app)
{
    GtkWidget* dialog;
    gchar* title;
    ...
    dialog = create_file_chooser(app->window.self, GTK_FILE_CHOOSER_ACTION_SAVE, title, NULL, NULL);
    g_free(title);
    g_signal_connect(dialog, "response", G_CALLBACK(on_export_text_file_response), app);
    gtk_widget_show(dialog);
}
...
void
ugtk_app_sequence_batch(UgtkApp* app)
{
    UgtkBatchDialog* bdialog;
    UgetNode* cnode;
    gchar* title;
...
void
ugtk_app_clipboard_batch(UgtkApp* app)
{
    UgtkBatchDialog* bdialog;
    UgtkSelectorPage* page;
    UgetNode* cnode;
    ...
    if (list == NULL) {
        ugtk_app_show_message(app, GTK_MESSAGE_ERROR, _("No URLs found in clipboard."));
        return;
    }
    // filter existing
    if (app->setting.ui.skip_existing) {
        if (ugtk_app_filter_existing(app, list) == 0) {
            ...
            g_list_free(list);
            ugtk_app_show_message(app, GTK_MESSAGE_INFO, _("All URLs had existed."));
            return;
        }
    }
...
int
ugtk_app_filter_existing(UgtkApp* app, GList* uris)
{
    int counts;
...
void
ugtk_app_download_changed(UgtkApp* app, UgetNode* dnode)
{
    GtkTreeModel* model;
    GtkTreePath* path;
    GtkTreeIter iter;
...
void
ugtk_app_category_changed(UgtkApp* app, UgetNode* cnode)
{
    GtkTreeIter iter;
    GtkTreePath* path;
    GtkTreeModel* model;
...
void
ugtk_app_add_default_category(UgtkApp* app)
{
    UgetNode* cnode;
    UgetCommon* common;
...
static void
on_message_response(GtkWidget* dialog, gint response, GtkWidget** value)
{
    gtk_widget_destroy(dialog);
    *value = NULL;
}
...
void
ugtk_app_show_message(UgtkApp* app, GtkMessageType type, const gchar* message)
{
    GtkWidget* dialog;
    GtkWidget** value;
    gchar* title;

    dialog = gtk_message_dialog_new(app->window.self, GTK_DIALOG_DESTROY_WITH_PARENT, type,
    ...
    g_free(title);
    // signal handler
    g_signal_connect(dialog, "response", G_CALLBACK(on_message_response), value);
    gtk_widget_show(dialog);
}
...
void
ugtk_clipboard_init(struct UgtkClipboard* clipboard, const gchar* pattern)
{
...
void
ugtk_clipboard_set_pattern(struct UgtkClipboard* clipboard, const gchar* pattern)
{
    if (clipboard->regex)
...
void
ugtk_clipboard_set_text(struct UgtkClipboard* clipboard, gchar* text)
{
...
GList*
ugtk_clipboard_get_uris(struct UgtkClipboard* clipboard)
{
    GList* list;
    gchar* text;
...
GList*
ugtk_clipboard_get_matched(struct UgtkClipboard* clipboard, const gchar* text)
{
    GList* link;
    GList* list;
    gchar* temp;
    ...
    for (link = list; link; link = link->next) {
        ...
        if (text == NULL || g_regex_match(clipboard->regex, text + 1, 0, NULL) == FALSE) {
            ...
            if (clipboard->website == FALSE || uget_site_get_id(link->data) == UGET_SITE_UNKNOWN) {
                g_free(link->data);
                link->data = NULL;
            }
        }
...
void
ugtk_statusbar_set_info(struct UgtkStatusbar* statusbar, gint n_selected)
{
...
void
ugtk_statusbar_set_speed(struct UgtkStatusbar* statusbar, gint64 down_speed, gint64 up_speed)
{
    char* string;
